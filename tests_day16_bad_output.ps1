...
$hdrPath  = ".\tmp_hdr_day16_bad.txt"
$bodyOut  = ".\tmp_body_day16_bad.json"

curl.exe -s -S -X POST `
  -H "Content-Type: application/json" `
  -H "X-Req-Id: $reqId" `
  -H "X-Dev-Force: ENGINE_BAD_OUTPUT" `
...
$okCode     = ($json.error_code -eq "ENGINE_BAD_OUTPUT")
$okReqMatch = ($hdrReq -and ($json.req_id -eq $hdrReq))

if ($okStatus -and $okCode -and $okReqMatch) {
  Write-Host "PASS: ENGINE_BAD_OUTPUT path ok; req_id=$($json.req_id) matches header"
  exit 0
} else {
  Write-Host "FAIL:"
  Write-Host "  status=='error'? $okStatus"
  Write-Host "  error_code=='ENGINE_BAD_OUTPUT'? $okCode (got: $($json.error_code))"
